...
@gin.configurable
def box_runner(num_boxes, box_frac, idx, T, max_epochs, batch_size, freq_print, genom, lr, weight_decay, num_samples, transfer_epochs, window_size=3):
    '''
...
def box_ising_temp():
    ...
    for idx, T in enumerate(Ts):
        ...
        print('='*50)
        current_mi = mice.box_runner(idx=idx, T=T)
        mi[idx] = current_mi
...
def box_caller(box_sizes):
     for idx, num_boxes in enumerate(box_sizes):
         box_runner(num_boxes=num_boxes, idx=idx, T=0.1)
